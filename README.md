...
| 3 | Dashboard rebuild: map, live video, mission planner, triage + CV Simulation Lab (L1/L2/L3) on the real pipeline | **Done (CV Lab)** — L1 replay/MP4, L2 procedural, L3 browser ingest; Agent Replay + Live CV | [Phase 3](docs/phase3/01-phase3-design-notes.md) · [Testing & Sim](docs/Testing-Simulation.md) |
| 4 | TensorRT edge deployment, Jetson Orin, real airframe — serving SLOs, INT8 re-certification, Isaac Sim digital twin | **Software ready / hardware pending** — runtime adapters, `bench/`, device-scoped gate profiles, ONNX export + Orin `export_tensorrt.sh`, sim regression CLI. Orin/INT8/Isaac/airframe still need hardware | [Phase 4](docs/phase4/01-inference-serving-and-cv-simulation-design.md) |
| 5 | Multi-tenant SaaS, billing, onboarding | **Scaffold** — tenancy helpers, billing stub (`profiles: [phase5]`), onboarding checklist. Stripe live / self-serve UI deferred to pre-deploy | [Phase 5](docs/phase5/01-phase5-design-notes.md) |
| 6 | DPIA, AI Act technical file, model cards, pen test | **Templates** — evidence pack under `docs/phase6/`. Legal sign-off + pen-test not claimed | [Phase 6](docs/phase6/README.md) |

> Roboflow remains **optional** (dataset labeling) — not required for lab or deploy path. See `.env.example`.
...
- **VisDrone model — 35/35 done** (`mAP@50 0.296` vs gate `0.30` — fail-closed). Artifact `data/models/visdrone-yolov8n.pt` is loadable in lab via compose `MODEL_PATH=/models/visdrone-yolov8n.pt` (mount `./data/models`) without promote; registry stamp stays `unregistered` until the gate passes. Extended fine-tune: `./scripts/train_extended.sh`.
- Evaluation harness + promotion gate: implemented and operational (fail-closed). Lab bypass for weight loading does not bypass certification.
- **Simulation lab — live on the real pipeline.** L1 = VisDrone corpus **or MP4** (`data/videos/`, build with `scripts/build_visdrone_mp4s.py`); L2 = procedural aerial with VisDrone class names (`pedestrian`/`car`/…); L3 browser 3D optional. Agent Replay runs **Live CV (YOLO)** beside the map. Regression: `scripts/sim_regression.py`. See `docs/Testing-Simulation.md`.
- **Phase 4 software in-repo:** inference runtime adapters (`runtimes.py`), stage profiler `bench/profile_stages.py`, device-scoped gate profiles (`mps-smoke` / `orin-trt-*`), ONNX export + `scripts/export_tensorrt.sh` (Orin-only). **Still hardware-blocked:** TensorRT INT8 on device, Isaac digital twin, real airframe.
- **Phase 5/6:** tenancy helpers + billing stub; compliance templates under `docs/phase6/`. Roboflow optional until pre-deploy.
- Dashboard otherwise functional. Helm/Terraform remain minimal; Compose is the supported deploy.
